...
const BottomSheets = ({
  ...
  const renderItem = ({ item, index }) => (
    ...
    >
      <View style={[styles.listItem, index === selectedItemIndex,themeMode == "dark" && { backgroundColor: "#282828",borderBottomColor: "#fff"}]}>
        <Text style={[{ fontSize: 17, fontWeight: "bold", flex: 1 },themeMode == "dark" && {color:"#fff" }]}>
          {item.Arabic}
        </Text>
        <Text
          style={[{
  ...
  return (
    <BottomSheet
      enablePanDownToClose={snapPoints}
      ...
      // backgroundStyle={[{borderRadius:0},themeMode === "dark" && {backgroundColor:"#282828"}]}
    >
      <BottomSheetFlatList
        ref={flatListRef}
        data={data}
